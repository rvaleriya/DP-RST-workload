...
# --- 1. Set up logging ---
logfile = open("STAMarker_STARmap_output.log", "w")
sys.stdout = logfile
sys.stderr = logfile
...
data_module.prepare_data(rad_cutoff=350, n_top_genes=2000, min_cells=0)
...
# Set accelerator to auto
config["stagate_trainer"]["accelerator"] = "auto"
config["classifier_trainer"]["accelerator"] = "auto"

# Initialize the STAMarker model
model = STAMarker(5, "STARmap_STAMarker_output/", config)
...
plt.tight_layout()
plt.savefig("STARmap_STAMarker.png")
...
output_df.to_csv("STAMarker_STARmap_joint.csv", index=False)