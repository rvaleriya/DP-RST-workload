...
in_csv   <- "starmap_pcs_coords_labels.csv"
...
run_kmeans_once <- function(df, n_pcs) {
  message(sprintf(">> Running k-means (K=8) with %d PCs ...", n_pcs))
  ...
  p_pred  <- plot_spatial(out_tbl, "label",
                          sprintf("k-means (K=8), %d PCs — Predicted Clusters", n_pcs))
  ...
  # Save labels to RDS
  rds_path <- file.path(out_dir, sprintf("KMeans_STARmap_%dPCs.rds", n_pcs))
...
res_3pc  <- run_kmeans_once(starmap, n_pcs = 3)
res_10pc <- run_kmeans_once(starmap, n_pcs = 10)
