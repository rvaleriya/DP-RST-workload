...
logfile = open("STAGATE_Lung_output.log", "w")
sys.stdout = logfile
sys.stderr = logfile
...
results_file = os.path.join(base_dir, "STAGATE_Lung.csv")
...
# Check Annotation_Type first as it's present in this dataset
...
if ground_truth_col:
    ...
    ari_file = os.path.join(base_dir, "STAGATE_Lung_ARI.txt")
...
# Create plot with three columns: 3PC clusters, 10PC clusters, true labels
if 'spatial' in adata.obsm:
    ...
    plt.tight_layout()
    plot_file = os.path.join(base_dir, "STAGATE_Lung_plot.png")
