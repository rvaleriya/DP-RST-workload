...
logfile_path = os.path.join(out_dir, "SpaMask_Lung_output.log")
...
print("=== SpaMask on Lung (PC=10; k=6) ===")
print(f"Reading data from CSV files in {out_dir}...")

# --------------------------- Load Converted CSV data ---------------------------------------
loc_df = pd.read_csv(os.path.join(out_dir, "lung_loc.csv"))
pcs_df = pd.read_csv(os.path.join(out_dir, "lung_pcs.csv"), index_col=0)
labels_df = pd.read_csv(os.path.join(out_dir, "lung_labels.csv"))
...
with open(os.path.join(out_dir, "lung_spamask_ari.txt"), "w") as f:
    f.write(f"pca={n_pcs}, k={num_clusters} -> ARI={ari_val:.6f}\n")
print("✓ Saved ARI summary to lung_spamask_ari.txt")

csv_out = os.path.join(out_dir, f"SpaMask_Lung_{n_pcs}PCs.csv")
