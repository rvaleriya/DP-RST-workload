###############################################################################
#                    BayesSpace on Lung Xenium (Radius Neighbors)
#   - Irregular coordinates (Xenium) handled via radius-based mutual graph
...
# ========================== 2. Paths & Data ==================================
work_dir <- "/scratch/user/varogovchenko/BayesSpace_runs/Lung"
...
load(data_rdata)
# Expects: counts_keep (dgCMatrix genes x cells)
#          pca_embeddings (cells x PCs, rownames = barcodes)
...
find_neighbors_radius <- function(xy, q_ref = 0.5, tau = 2, max_k = 50, mutual = TRUE) {
  stopifnot(is.matrix(xy) && ncol(xy) == 2)
  d1 <- FNN::get.knn(xy, k = 1)$nn.dist[, 1]
  r  <- as.numeric(tau * stats::quantile(d1, probs = q_ref, names = FALSE))
  nn <- RANN::nn2(xy, query = xy, k = max_k, radius = r, searchtype = "radius")
  ...
  message(sprintf("Neighbor graph: r=%.4f | mean deg=%.2f | mutual=%s | %s",
...
df_j <- find_neighbors_radius(xy_mat, q_ref = 0.5, tau = 2, max_k = 50, mutual = TRUE)
...
saveRDS(df_10, file = "BayesSpace_Lung_10PCs.rds")

p10 <- plot_spatial(df_10, "label",
                    sprintf("BayesSpace (Lung Xenium, 10 PCs)%s",
                            ifelse(is.na(ari_10), "", sprintf(", ARI = %.3f", ari_10))))
ggsave("BayesSpace_Lung_10PCs_clusters.png", p10, width = 6, height = 5, dpi = 300)
...
print(file_time)
cat("Finished BayesSpace runs on Lung Xenium.\n")
###############################################################################