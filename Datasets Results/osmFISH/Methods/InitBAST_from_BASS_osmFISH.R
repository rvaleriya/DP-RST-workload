# =========BASS on osmFISH for BAST initialization: 3PC & 10PC =================
# -----------------------------------------------------------------------------
set.seed(42)
...
sce_path <- "osmfish_sce_with_pca.rds"
out_dir  <- file.path(getwd(), "Results")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# ---- Load SCE ---------------------------------------------------------------
sce <- readRDS(sce_path)
assay(sce, "counts") <- assay(sce, "logcounts") # Patch for BASS functionality

# Counts (genes x cells) and coordinates (cells x 2)
count_mat <- as.matrix(logcounts(sce))              # genes x cells
...
run_bass_once_sce <- function(sce, n_pcs) {
  ...
  # Save
  cvs_path <- file.path(out_dir, sprintf("InitBAST_from_BASS_%dPCs.csv", n_pcs))
  write.csv(out_tbl, cvs_path, row.names = FALSE)
  
  list(labels_tbl = out_tbl, ari = ari, cvs_path = cvs_path, bass = bass)
}
...
cat("Saved RDS:\n")
cat("  - ", res_3pc$cvs_path,  "\n", sep = "")
cat("  - ", res_10pc$cvs_path, "\n", sep = "")