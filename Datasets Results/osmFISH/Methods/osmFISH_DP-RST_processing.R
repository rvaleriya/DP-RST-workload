# ==================== DP-RST post-processing: osmFISH (3PCs & 10PCs) ====================
...
bnd_all  <- read.csv(file.path(base_dir, "osmfish_manual_boundary.csv"))
...
process_one <- function(pc) {
  ...
  panel <- (p_true | p_teams) / (p_init | p_groups) +
    plot_annotation(title = sprintf("osmFISH — DP-RST Post-Processing (%d PCs)", pc),
                    theme = theme(plot.title = element_text(size = 12, face = "bold")))
  
  out_pdf <- file.path(results_dir, sprintf("osmFISH_DP-RST_Postproc_%dPCs.pdf", pc))
