##### DR-SC on osmFISH (simple version, fixed coordinate names) ###############
set.seed(42)
...
sce_rds <- "osmfish_sce_with_pca.rds"
...
var.features <- rownames(seu) 

# Extract data (Cells x Genes)
dat <- GetAssayData(seu, slot = "data")
X   <- Matrix::t(dat[var.features, ])
X   <- as.matrix(X) # DR.SC usually prefers dense matrix if small enough

# Check dimensions
message(paste("Matrix dimensions for DR.SC:", paste(dim(X), collapse=" x ")))
...
run_drsc_once <- function(seu_obj, q_dim, K = 11) {
  ...
  # Save
  rds_path <- file.path(out_dir, sprintf("DR-SC_osmFISH_%dPCs.rds", q_dim))
