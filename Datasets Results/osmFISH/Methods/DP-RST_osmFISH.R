#!/usr/bin/env Rscript
# ---------------------------------------------------------------------------
# Run DP.RST on osmFISH postproc files (3PCs & 10PCs) in parallel
# ---------------------------------------------------------------------------
t0 <- Sys.time()
cat("========== DP.RST osmFISH FULL inference started at",
    format(Sys.time()), "==========\n")
...
# ---------------------------------------------------------------------------
# 3. Locate osmFISH FULL postproc files --------------------------------------
# Expect exactly these two (created by your osmFISH postproc step):
...
run_full <- function(file_path) {
  ...
  # Loads (per your osmFISH postproc): Y_std, coords, graph0, init_val, hyperpar, clust_s (may vary)
  load(file_path)
  ...
  if (!exists("loc_full")) {
    if (!exists("coords")) stop("[FULL ", pcs, "PCs] coords missing in ", file_path)
    loc_full <- coords
  }
