...
logfile = open("STAGATE_osmFISH_output.log", "w")
sys.stdout = logfile
sys.stderr = logfile
...
num_clusters = 11  # True number of clusters
...
# Skip normalization and log transformation, keep all genes (all are HVGs for osmFISH)
...
results_file = os.path.join(base_dir, "STAGATE_osmFISH.csv")
...
if ground_truth_col:
    ...
    ari_file = os.path.join(base_dir, "STAGATE_osmFISH_ARI.txt")
...
if 'spatial' in adata.obsm:
    ...
    plt.tight_layout()
    plot_file = os.path.join(base_dir, "STAGATE_osmFISH_plot.png")
