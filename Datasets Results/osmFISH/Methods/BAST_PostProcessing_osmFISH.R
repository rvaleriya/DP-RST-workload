###############################################################################
##  POSTPROC for *osmFISH* (3PCs & 10PCs)
##  Preproc loads: Y_std, coords, graph0, init_val, hyperpar, clust_s
###############################################################################
...
# ---------- process the 3PCs & 10PCs files ----------
for (pc in c(3, 10)) {
  message("==> Processing osmFISH for ", pc, " PCs")
  ...
  # (2) Load preproc (Y_std, coords, graph0, init_val, hyperpar, clust_s)
  pre_file <- file.path(base_dir, sprintf("Preproc_BAST_%dPCs.RData", pc))
  ...
  save_list <- intersect(
    c("Y_std","coords","graph0","init_val","hyperpar","clust_s"),
    ls(envir = .GlobalEnv)
  )
...
message("🏁 Done: osmFISH files processed.")