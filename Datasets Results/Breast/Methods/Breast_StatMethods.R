...
# https://www.10xgenomics.com/datasets/human-breast-cancer-ductal-carcinoma-in-situ-invasive-carcinoma-ffpe-1-standard-1-3-0
...
### BayesSpace ###
library(BayesSpace)
sce_Breast <- readVisium("Breast/Space_Ranger_Data_Breast")

breast <- spatialPreprocess(sce_Breast)

# breast_tune <- qTune(breast, qs=seq(2, 20), d=3)
# qPlot(breast_tune)
...
for (c in clusters) {
  # Clustering with q=3
  breast_q3 <- spatialCluster(breast, q=c, d=3, platform='Visium',
                                init.method="mclust", model="t", gamma=3,
                                nrep=50000, burn.in=40000, save.chain=FALSE)

  # Store clustering labels
  df_q3 <- data.frame(
    imagerow = breast_q3$imagerow,
    imagecol = breast_q3$imagecol,
    spatial_cluster = breast_q3$spatial.cluster,
    PCs = 3,
    q = c
  )
  ...
  breast_q10 <- spatialCluster(breast, q=c, d=10, platform='Visium',
                                 init.method="mclust", model="t", gamma=3,
                                 nrep=50000, burn.in=40000, save.chain=FALSE)

  # Store clustering labels
  df_q10 <- data.frame(
    imagerow = breast_q10$imagerow,
    imagecol = breast_q10$imagecol,
    spatial_cluster = breast_q10$spatial.cluster,
    PCs = 10,
    q = c
  )
...
saveRDS(merged_df, file = "Breast/BayesSpace_Breast_clustering_results.rds")
...
breast_merged_df <- merge(ground_truth_df, merged_df_renamed,
                         by = c("x", "y"),
                         all = TRUE)

breast_subset <- subset(breast_merged_df, !is.na(breast_merged_df$z))
head(breast_subset)

clustering_cols <- grep("^cluster_", names(breast_subset), value = TRUE)

ari_scores <- sapply(clustering_cols, function(col) {
  adjustedRandIndex(breast_subset$z, breast_subset[[col]])
})
...
print(ari_pc3)
# Clustering       ARI q PC
# cluster_q7_pc3 cluster_q7_pc3 0.5511279 7  3
# cluster_q5_pc3 cluster_q5_pc3 0.5465307 5  3
# cluster_q3_pc3 cluster_q3_pc3 0.5239261 3  3

print(ari_pc10)
# Clustering       ARI q PC
# cluster_q5_pc10 cluster_q5_pc10 0.4392270 5 10
# cluster_q3_pc10 cluster_q3_pc10 0.3328900 3 10
# cluster_q7_pc10 cluster_q7_pc10 0.3124531 7 10
...
breast_SC.MEB_3pc = SC.MEB(Y[, 1:3], Adj_sp, beta_grid = beta_grid, K_set = K_set,
                          parallel = parallel, num_core = num_core, PX = PX,
                          maxIter_ICM = maxIter_ICM, maxIter = maxIter)
str(breast_SC.MEB_3pc[,1])
str(breast_SC.MEB_3pc[,2])
str(breast_SC.MEB_3pc[,3])

breast_SC.MEB_10pc = SC.MEB(Y[, 1:10], Adj_sp, beta_grid = beta_grid, K_set = K_set,
                           parallel = parallel, num_core = num_core, PX = PX,
                           maxIter_ICM = maxIter_ICM, maxIter = maxIter)
...
# selectKPlot(breast_fit, K_set = K_set, criterion = "BIC")
#
# #### Selecting the number of clusters using Modified BIC
# selectKPlot(breast_fit, K_set = K_set, criterion = "MBIC")
#
# out = selectK(breast_fit, K_set = K_set, criterion = "BIC")
# ClusterPlot(out, loc)
#
# breast_SC.MEB_df <- data.frame(loc, out[["best_K_label"]], z)

breast_SC.MEB_df <- data.frame(loc,
                              pc3_k3 = breast_SC.MEB_3pc[,1]$x,
                              pc3_k5 = breast_SC.MEB_3pc[,2]$x,
                              pc3_k7 = breast_SC.MEB_3pc[,3]$x,
                              pc10_k3 = breast_SC.MEB_10pc[,1]$x,
                              pc10_k5 = breast_SC.MEB_10pc[,2]$x,
                              pc10_k7 = breast_SC.MEB_10pc[,3]$x,
                              z)
head(breast_SC.MEB_df)
saveRDS(breast_SC.MEB_df, file = "Breast/SC-MEB_Breast_clustering_results.rds")

breast_subset <- subset(breast_SC.MEB_df, !is.na(z))
head(breast_subset)

clustering_cols <- grep("^pc", names(breast_subset), value = TRUE)

ari_scores <- sapply(clustering_cols, function(col) {
  round(adjustedRandIndex(breast_subset$z, breast_subset[[col]]), 3)
})
...
# Clustering   ARI  PC K
# pc3_k5     pc3_k5 0.577 pc3 5
# pc3_k3     pc3_k3 0.549 pc3 3
# pc3_k7     pc3_k7 0.492 pc3 7

print(ari_pc10)
# Clustering   ARI   PC K
# pc10_k3    pc10_k3 0.521 pc10 3
# pc10_k5    pc10_k5 0.434 pc10 5
# pc10_k7    pc10_k7 0.337 pc10 7


ggplot(breast_subset, aes(x = x, y = y, color = as.factor(pc3_k5))) +
  geom_point(size = 1) +
...
# Create count matrix
data_dir_Breast_count <- 'Breast/Space_Ranger_Data_Breast/filtered_feature_bc_matrix'
list.files(data_dir_Breast_count) # Should show barcodes.tsv, genes.tsv, and matrix.mtx
expression_matrix <- Read10X(data.dir = data_dir_Breast_count)

# Load spatial data
tissue_positions_list <- readr::read_csv("Breast/Space_Ranger_Data_Breast/spatial/tissue_positions_list.csv",
                                  col_names = FALSE)
...
Breast <- NormalizeData(seurat_object, verbose = F)
# choose 2000 highly variable features
seu <- FindVariableFeatures(Breast, nfeatures = 2000, verbose = F)
...
# Merge all dataframes by coordinates
merged_df <- purrr::reduce(c(df_list_q3, df_list_q10), full_join, by = c("imagerow", "imagecol"))
saveRDS(merged_df, file = "Breast/DR-SC_Breast_clustering_results.rds")
...
print(ari_pc3)
# Clustering       ARI q PC
# cluster_q3_pc3 cluster_q3_pc3 0.5403865 3  3
# cluster_q5_pc3 cluster_q5_pc3 0.5128676 5  3
# cluster_q7_pc3 cluster_q7_pc3 0.4454788 7  3

print(ari_pc10)
# Clustering       ARI q PC
# cluster_q3_pc10 cluster_q3_pc10 0.4790993 3 10
# cluster_q5_pc10 cluster_q5_pc10 0.4285450 5 10
# cluster_q7_pc10 cluster_q7_pc10 0.3434748 7 10

ggplot(breast_subset, aes(x = x, y = y, color = as.factor(cluster_q5_pc3))) +
  geom_point(size = 1) +
  scale_color_discrete(name = "Cluster") +
  coord_fixed() +  # keep aspect ratio
  theme_minimal() +
...
breast_kmeans_df <- data.frame(loc,
...
saveRDS(breast_kmeans_df, file = "Breast/kmeans_Breast_clustering_results.rds")

breast_subset <- subset(breast_kmeans_df, !is.na(z))
head(breast_subset)
...
print(ari_pc3)
# Clustering   ARI  PC K
# pc3_k3     pc3_k3 0.492 pc3 3
# pc3_k5     pc3_k5 0.407 pc3 5
# pc3_k7     pc3_k7 0.294 pc3 7

print(ari_pc10)
# Clustering   ARI   PC K
# pc10_k3    pc10_k3 0.492 pc10 3
# pc10_k5    pc10_k5 0.326 pc10 5
# pc10_k7    pc10_k7 0.315 pc10 7
