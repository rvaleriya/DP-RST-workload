...
Breast_BS_results <- readRDS("/Breast/NewRuns_2025/BayesSpace_Breast_clustering_results.rds")
colnames(Breast_BS_results)[colnames(Breast_BS_results) == "imagerow"] <- "x"
colnames(Breast_BS_results)[colnames(Breast_BS_results) == "imagecol"] <- "y"
head(Breast_BS_results)
...
coords_init <- loc %>%
  left_join(Breast_BS_results, by = c("x", "y"))
...
dynamic_part <- "Breast_BAST_PT_Version2_1_50k_diff01_newBS"

print("All the parameters are set")

start.time_mcmc <- Sys.time()
Breast_BAST_PT_Version2_1_50k_diff01_newBS = fitBAST(Y_std, graph0, init_val, hyperpar, temp,
                                MCMC, BURNIN, THIN,
                                PT = TRUE, seed = 7394,
                                backup_d = sprintf("/Breast/NewRuns_2025/Outputs_BAST/%s_backup.RData", dynamic_part))
end.time_mcmc <- Sys.time()

mcmc_time <- end.time_mcmc - start.time_mcmc
mcmc_time

save(Breast_BAST_PT_Version2_1_50k_diff01_newBS, file = "/Breast/NewRuns_2025/Outputs_BAST/Breast_BAST_PT_Version2_1_50k_diff01_newBS_OutputOnly.RData")
