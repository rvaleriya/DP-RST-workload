...
# The output directory is the same as the script's directory.
out_dir = os.path.dirname(os.path.realpath(__file__))
os.makedirs(out_dir, exist_ok=True)

logfile_path = os.path.join(out_dir, "SpaMask_Breast_output.log")
...
print("=== SpaMask on Breast (PC=10; k=5) ===")
print(f"Reading data from CSV files in {out_dir}...")

# --------------------------- Load Converted CSV data ---------------------------------------
loc_df = pd.read_csv(os.path.join(out_dir, "breast_loc.csv"))
pcs_df = pd.read_csv(os.path.join(out_dir, "breast_pcs.csv"))
labels_df = pd.read_csv(os.path.join(out_dir, "breast_labels.csv"))
...
with open(os.path.join(out_dir, "breast_spamask_ari.txt"), "w") as f:
    f.write(f"pca={n_pcs}, k={num_clusters} -> ARI={ari_val:.6f}\n")
print("✓ Saved ARI summary to breast_spamask_ari.txt")

csv_out = os.path.join(out_dir, f"SpaMask_Breast_{n_pcs}PCs.csv")
