...
setwd("~/Desktop/DP-RST-workload/Datasets Results/Breast")

load("10x_breast_cancer_iIMPACT_data.RData")
load("breast_boundary.RData")
...
#-------------------------------------------------------------------------------
file_path <- "./Results/Breast_DP.RST_newBS_p3_Version2_OutputOnly.RData"
load(file_path)
output = Breast_DP.RST_newBS_p3_Version2

file_path_bast <- "./Results/Breast_BAST_PT_Version2_1_50k_diff01_newBS_OutputOnly.RData"
load(file_path_bast)
output_bast = Breast_BAST_PT_Version2_1_50k_diff01_newBS
...
cluster_count_hist <- ggplot(teams_freq, aes(x = Var1, y = Freq)) +
  geom_bar(stat = "identity", color = "#9E8CCF", fill = "#E6E1FA", width = 0.8) +
  labs(title = "Human Breast Cancer",
...
ggsave(filename = paste0(getwd(), "/Results/hist_MCMC_clusters_Breast.png"),
...
for (i in 1:length(output[["marginal_likelihood_out"]])){
  Breast_df <- data.frame(res_memb = output[["cluster_out"]][i,], z)
  Breast_df <- subset(Breast_df, !is.na(z))
  
  accur_spatial[i] <- mclust::adjustedRandIndex(Breast_df$res_memb,
                                                Breast_df$z)
  ...
  Breast_df <- data.frame(res_memb = obs_in_teams_vec, z)
  Breast_df <- subset(Breast_df, !is.na(z))
  
  accur_refined[i] = mclust::adjustedRandIndex(Breast_df$res_memb, Breast_df$z)
}
...
Breast_df <- data.frame(res_1 = mode_based_partition$groups_partition,
...
Breast_df <- data.frame(res_groups_1 = mode_based_partition$groups_partition,
...
accur_groups_1 = mclust::adjustedRandIndex(Breast_df$res_groups_1, Breast_df$z)
accur_groups_1

accur_groups_2 = mclust::adjustedRandIndex(Breast_df$res_groups_2, Breast_df$z)
accur_groups_2

accur_teams_1 = mclust::adjustedRandIndex(Breast_df$res_teams_1, Breast_df$z)
accur_teams_1

accur_teams_2 = mclust::adjustedRandIndex(Breast_df$res_teams_2, Breast_df$z)
accur_teams_2
...
Breast_BS_results <- readRDS("Breast/BayesSpace_Breast_clustering_results.rds")

Breast_BS_results <- Breast_BS_results %>%
  rename(x = imagerow, y = imagecol)

Breast_BS_results_merged <- merge(ground_truth_df, Breast_BS_results,
                                 by = c("x", "y"),
                                 all = TRUE)

Breast_BS_results_subset <- subset(Breast_BS_results_merged, !is.na(Breast_BS_results_merged$z))

accur_BS = mclust::adjustedRandIndex(Breast_BS_results_subset$cluster_q5_pc3, Breast_BS_results_subset$z)
accur_BS


BS_res <- groups_plot(Breast_BS_results[, c("x", "y")], Breast_BS_results$cluster_q5_pc3, bnd) +
  theme(legend.position = "none") +
  ggtitle(paste("BayesSpace results (at truth), k = ", length(unique(Breast_BS_results_subset$cluster_q3_pc3)), " , ARI = ",
                round(accur_BS, 3),
                sep = ""))
# BS_res
...
init_partition <- compute_MST_spatial(Breast_BS_results,
                                      coords_cols = c("x", "y"),
                                      cluster_col = "cluster_q5_pc3",
                                      bnd = bnd)

init_spatial <- groups_plot(Breast_BS_results[, c("x", "y")], init_partition$data$spatial_cluster, bnd) +
  theme(legend.position = "none") +
...
title_text <- sprintf("Breast, DP-RST with PT, PCs = %d, diff = 0.1. Initialize at new BAST PT from spatial partition of BayesSpace (at truth) output.
...
save_path <- paste0("Breast/DP-RST_Outputs/", save_file_name, ".pdf")
