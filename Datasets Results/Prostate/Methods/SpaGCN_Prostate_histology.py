...
# Logging
# =========================
LOGFILE = "SpaGCN_Prostate_histology_output.log"
...
OUTPUT_CSV = "SpaGCN_Prostate_histology.csv"
OUTPUT_ARI_TXT = "SpaGCN_Prostate_histology_ARI.txt"
OUTPUT_PLOT = "SpaGCN_Prostate_histology_plot.png"
...
if ari_rows:
    ...
    with open(OUTPUT_ARI_TXT, "w") as f:
        f.write("SpaGCN on Prostate (k=3) WITH HISTOLOGY — ARI summary\n")
        f.write("="*60 + "\n")
...
sc.pl.spatial(adata_plot_base, color="refined_3PCs", ax=axes[0], show=False, 
              title=f'Prostate - 3PCs (k={N_CLUSTERS}) [Histology]', spot_size=120)

# 10PCs results
sc.pl.spatial(adata_plot_base, color="refined_10PCs", ax=axes[1], show=False, 
              title=f'Prostate - 10PCs (k={N_CLUSTERS}) [Histology]', spot_size=120)
