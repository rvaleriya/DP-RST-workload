...
# Logging
# =========================
LOGFILE = "SpaGCN_Prostate_output.log"
...
OUTPUT_CSV = "SpaGCN_Prostate.csv"
OUTPUT_ARI_TXT = "SpaGCN_Prostate_ARI.txt"
OUTPUT_PLOT = "SpaGCN_Prostate_plot.png"
...
if ari_rows:
    ...
    with open(OUTPUT_ARI_TXT, "w") as f:
        f.write("SpaGCN on Prostate (k=3) — ARI summary\n")
        f.write("="*60 + "\n")
...
adata_plot_base.obs["refined_3PCs"] = results_indexed.loc[adata_plot_base.obs_names, "refined_pred_3PCs_k3"].astype('category')
adata_plot_base.obs["refined_10PCs"] = results_indexed.loc[adata_plot_base.obs_names, "refined_pred_10PCs_k3"].astype('category')
...
sc.pl.spatial(adata_plot_base, color="refined_3PCs", ax=axes[0], show=False, 
              title=f'Prostate - 3PCs (k={N_CLUSTERS})', spot_size=120)

# 10PCs results
sc.pl.spatial(adata_plot_base, color="refined_10PCs", ax=axes[1], show=False, 
              title=f'Prostate - 10PCs (k={N_CLUSTERS})', spot_size=120)
    ...
    if ground_truth_col == 'true_labels':
        ...
        if aligned_mask.sum() > 0:
            ...
            sc.pl.spatial(temp_adata, color=ground_truth_col, ax=axes[2], 
                         show=False, title='Prostate - True Labels', spot_size=120)
        else:
            axes[2].text(0.5, 0.5, 'No aligned ground truth\nlabels available', 
                       ha='center', va='center', transform=axes[2].transAxes)
            axes[2].set_title('Prostate - True Labels (N/A)')
    else:
        sc.pl.spatial(adata_plot_base, color=ground_truth_col, ax=axes[2], show=False, 
                     title='Prostate - True Labels', spot_size=120)
else:
