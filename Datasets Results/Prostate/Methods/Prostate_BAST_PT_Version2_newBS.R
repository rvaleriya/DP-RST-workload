...
Prostate_BS_results <- readRDS("/Prostate/NewRuns_2025/BayesSpace_Prostate_clustering_results.rds")
colnames(Prostate_BS_results)[colnames(Prostate_BS_results) == "imagerow"] <- "x"
colnames(Prostate_BS_results)[colnames(Prostate_BS_results) == "imagecol"] <- "y"
head(Prostate_BS_results)
...
coords_init <- loc %>%
  left_join(Prostate_BS_results, by = c("x", "y"))
...
load("/Prostate/prostate_mesh.RData")
...
dynamic_part <- "Prostate_BAST_PT_Version2_newBS"

print("All the parameters are set")

start.time_mcmc <- Sys.time()
Prostate_BAST_PT_Version2_newBS = fitBAST(Y_std, graph0, init_val, hyperpar, temp,
                                MCMC, BURNIN, THIN,
                                PT = TRUE, seed = 7394,
                                backup_d = sprintf("Prostate/NewRuns_2025/Outputs_BAST/%s_backup.RData", dynamic_part))
end.time_mcmc <- Sys.time()

mcmc_time <- end.time_mcmc - start.time_mcmc
mcmc_time

# save(Prostate_BAST_PT_Version2, file = "Prostate/NewRuns_2025/Outputs_BAST/Prostate_BAST_PT_Version2_OutputOnly.RData")
saveRDS(Prostate_BAST_PT_Version2_newBS, file = "/Prostate/NewRuns_2025/Outputs_BAST/Prostate_BAST_PT_Version2_newBS_OutputOnly.rds")
