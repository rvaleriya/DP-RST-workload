...
#-------------------------------------------------------------------------------
setwd("~/Desktop/DP-RST-workload/Datasets Results/Prostate")

load("10x_prostate_cancer_iIMPACT_data.RData")
load("prostate_boundary.RData")
...
file_path <- "Results/Prostate_DP.RST_newBS_p3_Version2_OutputOnly.rds"
output = readRDS(file_path)

file_path_bast <- "Results/Prostate_BAST_PT_Version2_newBS_OutputOnly.rds"
...
cluster_count_hist <- ggplot(teams_freq, aes(x = Var1, y = Freq)) +
  ...
  labs(title = "Human Prostate Cancer",
...
ggsave(filename = paste0(getwd(), "/Results/hist_MCMC_clusters_Prostate.png"),
