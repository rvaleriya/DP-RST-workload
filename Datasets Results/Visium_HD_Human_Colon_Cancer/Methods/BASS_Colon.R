# ========================== BASS on Colon Visium HD (10 PCs) ==========================
...
# Paths
setwd("/scratch/user/varogovchenko/BASS_runs/Colon")
load("/scratch/user/varogovchenko/BASTION_HPRC/Visium_HD_Human_Colon_Cancer/Nuclei_Data/colon_hd_nuclei_data_processed.RData")

# ---- Align barcodes across counts / PCs / spatial coords -----------------------
barcodes <- colnames(counts_keep)              # master order
Y        <- pca_embeddings[barcodes, , drop=FALSE]
xy_mat   <- as.matrix(
  spatial_keep[match(barcodes, spatial_keep$cell_id),
               c("x","y")]
)
rownames(xy_mat) <- barcodes

z <- spatial_keep$annotation
