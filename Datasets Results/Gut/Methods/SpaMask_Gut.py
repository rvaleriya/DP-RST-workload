...
logfile_path = os.path.join(out_dir, "SpaMask_Gut_output.log")
...
print("=== SpaMask on Gut (PC=10; k=5) ===")
print(f"Reading data from CSV files in {out_dir}...")

# --------------------------- Load Converted CSV data ---------------------------------------
df = pd.read_csv(os.path.join(out_dir, "gut_data.csv"), index_col=0)
...
features = df[pc_cols].values

# Rename 'z' column to 'ground_truth' for consistency
df.rename(columns={'z': 'ground_truth'}, inplace=True)

adata = ad.AnnData(np.zeros((n, 1), dtype=np.float32))
adata.obs_names = df.index.astype(str).values
...
with open(os.path.join(out_dir, "gut_spamask_ari.txt"), "w") as f:
    f.write(f"pca={n_pcs}, k={num_clusters} -> ARI={ari_val:.6f}\n")
print("✓ Saved ARI summary to gut_spamask_ari.txt")

csv_out = os.path.join(out_dir, f"SpaMask_Gut_{n_pcs}PCs.csv")
