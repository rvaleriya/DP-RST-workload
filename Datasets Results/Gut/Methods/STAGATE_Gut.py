...
logfile = open("STAGATE_Gut_output.log", "w")
sys.stdout = logfile
sys.stderr = logfile
...
results_file = os.path.join(base_dir, "STAGATE_Gut.csv")
...
if not ground_truth_col:
    ...
    elif os.path.exists(true_labels_csv_reduced):
...
if ground_truth_col:
    ...
    if loaded_from_csv and ground_truth_col == 'true_labels':
        ...
        if valid_mask.sum() > 0:
            ...
            print("No valid ground truth labels found - skipping ARI")
    
    ari_file = os.path.join(base_dir, "STAGATE_Gut_ARI.txt")
...
plt.tight_layout()
plot_file = os.path.join(base_dir, "STAGATE_Gut_plot.png")
