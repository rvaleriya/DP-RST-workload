...
# --- 1. Set up logging ---
logfile = open("STAMarker_Gut_output.log", "w")
sys.stdout = logfile
sys.stderr = logfile

# --- 2. Load and preprocess data ---
base_path = "../../ST_Datasets/Gut_reduced"
ann_data = sc.read_visium(base_path)

print("Subsetting Gut_reduced to selected spots...")
subset_csv = os.path.join(base_path, "gut_df_wt_muscle_rownames.csv")
spot_ids = pd.read_csv(subset_csv, header=None).squeeze().astype(str)
ann_data = ann_data[ann_data.obs_names.isin(spot_ids)].copy()
print(f"✓ Subsetted to {ann_data.n_obs} spots")
print(ann_data)
...
model = STAMarker(5, "Gut_STAMarker_output/", config)
...
all_true_labels_df = pd.read_csv("../../ST_Datasets/Gut_reduced/gut_true_labels.csv")
...
# Save to CSV
output_df.to_csv("STAMarker_Gut_joint.csv", index=False)