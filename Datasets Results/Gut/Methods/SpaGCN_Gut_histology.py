...
# Logging
# =========================
LOGFILE = "SpaGCN_Gut_histology_output.log"
...
OUTPUT_CSV = "SpaGCN_Gut_histology.csv"
OUTPUT_ARI_TXT = "SpaGCN_Gut_histology_ARI.txt"
OUTPUT_PLOT = "SpaGCN_Gut_histology_plot.png"
...
gut_reduced_path = BASE_DATA_DIR / "Gut_reduced"
subset_csv = gut_reduced_path / "gut_df_wt_muscle_rownames.csv"
if subset_csv.exists():
    ...
    print(f"Warning: Subset CSV not found at {subset_csv}, proceeding with all spots")
...
true_labels_csv = DATASET_PATH / "gut_true_labels.csv"
gut_reduced_path = BASE_DATA_DIR / "Gut_reduced"
true_labels_csv_reduced = gut_reduced_path / "gut_true_labels.csv"
...
if not ground_truth_col:
    ...
    elif true_labels_csv_reduced.exists():
...
for num_pcs in NUM_PCS_LIST:
    ...
    clf = spg.SpaGCN()
    # l must be set before training (SpaGCN requirement)
    if l is None:
...
if ari_rows:
    ...
    with open(OUTPUT_ARI_TXT, "w") as f:
        f.write("SpaGCN on Gut (k=5) WITH HISTOLOGY — ARI summary\n")
        f.write("="*60 + "\n")
...
if subset_csv.exists():
    spot_ids = pd.read_csv(subset_csv, header=None).squeeze().astype(str)
    adata_plot_base = adata_plot_base[adata_plot_base.obs_names.isin(spot_ids)].copy()
...
sc.pl.spatial(adata_plot_base, color="refined_3PCs", ax=axes[0], show=False, 
              title=f'Gut - 3PCs (k={N_CLUSTERS}) [Histology]', spot_size=120)

# 10PCs results
sc.pl.spatial(adata_plot_base, color="refined_10PCs", ax=axes[1], show=False, 
              title=f'Gut - 10PCs (k={N_CLUSTERS}) [Histology]', spot_size=120)
