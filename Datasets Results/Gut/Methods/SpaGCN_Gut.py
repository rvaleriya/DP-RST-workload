...
OUTPUT_CSV = "SpaGCN_Gut.csv"
OUTPUT_ARI_TXT = "SpaGCN_Gut_ARI.txt"
OUTPUT_PLOT = "SpaGCN_Gut_plot.png"
...
adata.obs["x_array"] = adata.obs["array_col"].tolist()
adata.obs["y_array"] = adata.obs["array_row"].tolist()
...
if ari_rows:
    ...
    with open(OUTPUT_ARI_TXT, "w") as f:
        f.write("SpaGCN on Gut (k=5) — ARI summary\n")
        f.write("="*60 + "\n")
...
sc.pl.spatial(adata_plot_base, color="refined_3PCs", ax=axes[0], show=False, 
              title=f'Gut - 3PCs (k={N_CLUSTERS})', spot_size=120)

# 10PCs results
sc.pl.spatial(adata_plot_base, color="refined_10PCs", ax=axes[1], show=False, 
              title=f'Gut - 10PCs (k={N_CLUSTERS})', spot_size=120)
