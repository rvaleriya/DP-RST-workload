...
logfile_path = os.path.join(out_dir, "SpaMask_Brain_output.log")
...
print("=== SpaMask on Brain (PC=10; k=7) ===")
print(f"Reading data from CSV files in {out_dir}...")

# --------------------------- Load Converted CSV data ---------------------------------------
loc_df = pd.read_csv(os.path.join(out_dir, "brain_loc.csv"))
pcs_df = pd.read_csv(os.path.join(out_dir, "brain_pcs.csv"))
labels_df = pd.read_csv(os.path.join(out_dir, "brain_labels.csv"))
...
with open(os.path.join(out_dir, "brain_spamask_ari.txt"), "w") as f:
    f.write(f"pca={n_pcs}, k={num_clusters} -> ARI={ari_val:.6f}\n")
print("✓ Saved ARI summary to brain_spamask_ari.txt")

csv_out = os.path.join(out_dir, f"SpaMask_Brain_{n_pcs}PCs.csv")
