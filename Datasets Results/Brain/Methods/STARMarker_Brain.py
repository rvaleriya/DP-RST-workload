...
# --- 1. Set up logging ---
logfile = open("STARMarker_Brain_output.log", "w")
sys.stdout = logfile
sys.stderr = logfile

# --- 2. Load and preprocess data ---
base_path = "../../ST_Datasets/Brain"
ann_data = sc.read_visium(base_path)
print(ann_data)
data_module = make_spatial_data(ann_data)
data_module.prepare_data(rad_cutoff=150, n_top_genes=2000, min_cells=0)
...
model = STAMarker(5, "Brain_STARMarker_output/", config)
...
all_true_labels_df = pd.read_csv("../../ST_Datasets/Brain/brain_true_labels.csv")
...
consensus_aligned = ann_data.obs["Consensus clustering"].iloc[indices].to_numpy().astype(int)
true_aligned = ari_true_labels_df['type'].to_numpy().astype(int)
...
output_df = pd.merge(
    ...
    on=['x', 'y'],
    how='left'
).rename(columns={'type': 'true_label'})

# Save to CSV
output_df.to_csv("STAMarker_Brain_joint.csv", index=False)