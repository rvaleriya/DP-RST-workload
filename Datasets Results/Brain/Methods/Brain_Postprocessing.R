...
setwd("~/Desktop/DP-RST-workload/Datasets Results/Brain")

#-------------------------------------------------------------------------------

load("DFPLC_151510_data_for_iIMPACT.RData")
load("brain_boundary.RData")
...
file_path <- "Results/Brain_DP.RST_FromNewBastPT_newBS_p3_Version2_OutputOnly.rds"
# load(file_path)
# output = Brain_DP.RST_FromBackup_Version2
output = readRDS(file_path)

file_path_bast <- "Results/Brain_BAST_PT_Version1_newBS_OutputOnly.rds"
# load(file_path_bast)
# output_bast = Brain_DP.RST_FromBackup_Version2
...
cluster_count_hist <- ggplot(teams_freq, aes(x = Var1, y = Freq)) +
  ...
  labs(title = "Human DLPFC (Sample #151510)",
...
ggsave(filename = paste0(getwd(), "/Results/hist_MCMC_clusters.png"),
...
for (i in 1:length(output[["marginal_likelihood_out"]])){
  Brain_df <- data.frame(res_memb = output[["cluster_out"]][i,], z)
  Brain_df <- subset(Brain_df, !is.na(z))

  accur_spatial[i] <- mclust::adjustedRandIndex(Brain_df$res_memb,
                                                Brain_df$z)
  ...
  Brain_df <- data.frame(res_memb = obs_in_teams_vec, z)
  Brain_df <- subset(Brain_df, !is.na(z))

  accur_refined[i] = mclust::adjustedRandIndex(Brain_df$res_memb, Brain_df$z)
}
...
Brain_df <- data.frame(res_1 = mode_based_partition$groups_partition,
...
Brain_df <- data.frame(res_groups_1 = mode_based_partition$groups_partition,
...
accur_groups_1 = mclust::adjustedRandIndex(Brain_df$res_groups_1, Brain_df$z)
accur_groups_1

accur_groups_2 = mclust::adjustedRandIndex(Brain_df$res_groups_2, Brain_df$z)
accur_groups_2

accur_teams_1 = mclust::adjustedRandIndex(Brain_df$res_teams_1, Brain_df$z)
accur_teams_1

accur_teams_2 = mclust::adjustedRandIndex(Brain_df$res_teams_2, Brain_df$z)
accur_teams_2
...
Brain_BS_results <- readRDS("Brain/BayesSpace_Brain_clustering_results.rds")

Brain_BS_results <- Brain_BS_results %>%
  rename(x = imagerow, y = imagecol)

Brain_BS_results_merged <- merge(ground_truth_df, Brain_BS_results,
                         by = c("x", "y"),
                         all = TRUE)

Brain_BS_results_subset <- subset(Brain_BS_results_merged, !is.na(Brain_BS_results_merged$type))

accur_BS = mclust::adjustedRandIndex(Brain_BS_results_subset$cluster_q7_pc3, Brain_BS_results_subset$type)
accur_BS


BS_res <- groups_plot(Brain_BS_results[, c("x", "y")], Brain_BS_results$cluster_q7_pc3, bnd) +
  theme(legend.position = "none") +
  ggtitle(paste("BayesSpace results (at truth), k = ", length(unique(Brain_BS_results$cluster_q7_pc3)), " , ARI = ",
                round(accur_BS, 3),
                sep = ""))
BS_res
...
init_partition <- compute_MST_spatial(Brain_BS_results,
                                      coords_cols = c("x", "y"),
                                      cluster_col = "cluster_q7_pc3",
                                      bnd = bnd)

init_spatial <- groups_plot(Brain_BS_results[, c("x", "y")], init_partition$data$spatial_cluster, bnd) +
  theme(legend.position = "none") +
...
title_text <- sprintf("Brain, DP-RST with PT, PCs = %d, diff = 0.1. Initialize at new BAST PT from spatial partition of BayesSpace (at truth) output.
...
save_path <- paste0("Brain/DP-RST_Outputs/", save_file_name, ".pdf")
