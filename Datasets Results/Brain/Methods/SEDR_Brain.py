...
logfile = open("SEDR_Brain_output.log", "w")
sys.stdout = logfile
sys.stderr = logfile
...
ari_results = {}
gt_csv_path = os.path.join(dataset_path, "brain_true_labels.csv")
...
if os.path.exists(gt_csv_path):
    try:
        ...
        for pcs, cluster_key in cluster_keys:
            ...
            true_aligned = ari_true_labels_df['type'].to_numpy()
...
if ari_results:
    ...
    with open("brain_sedr_ari.txt", "w") as f:
        for line in ari_lines:
            f.write(line + "\n")
    print("✓ Saved ARI summary to brain_sedr_ari.txt")
