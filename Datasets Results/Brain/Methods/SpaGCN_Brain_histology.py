...
# Logging
# =========================
LOGFILE = "SpaGCN_Brain_histology_output.log"
...
OUTPUT_CSV = "SpaGCN_Brain_histology.csv"
OUTPUT_ARI_TXT = "SpaGCN_Brain_histology_ARI.txt"
OUTPUT_PLOT = "SpaGCN_Brain_histology_plot.png"
...
true_labels_csv = DATASET_PATH / "brain_true_labels.csv"
...
if not ground_truth_col:
    if true_labels_csv.exists():
        ...
        if len(ari_true_labels_df) > 0:
            ...
            aligned_labels = ari_true_labels_df['type'].astype(str).values
...
if ari_rows:
    ...
    with open(OUTPUT_ARI_TXT, "w") as f:
        f.write("SpaGCN on Brain (k=7) WITH HISTOLOGY — ARI summary\n")
        f.write("="*60 + "\n")
...
adata_plot_base.obs["refined_3PCs"] = results_indexed.loc[adata_plot_base.obs_names, "refined_pred_3PCs_k7"].astype('category')
adata_plot_base.obs["refined_10PCs"] = results_indexed.loc[adata_plot_base.obs_names, "refined_pred_10PCs_k7"].astype('category')
...
sc.pl.spatial(adata_plot_base, color="refined_3PCs", ax=axes[0], show=False, 
              title=f'Brain - 3PCs (k={N_CLUSTERS}) [Histology]', spot_size=120)

# 10PCs results
sc.pl.spatial(adata_plot_base, color="refined_10PCs", ax=axes[1], show=False, 
              title=f'Brain - 10PCs (k={N_CLUSTERS}) [Histology]', spot_size=120)
