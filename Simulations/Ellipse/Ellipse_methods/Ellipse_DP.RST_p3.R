...
load("/Ellipse_sim_BAST_p3_30reps.RData")
...
seeds <- c()
for (i in 1:length(Ellipse_sim_BAST_p3_reps)){
  seeds[i] <- Ellipse_sim_BAST_p3_reps[[i]][["seed"]]
}
seeds

print("Starting the loop")

Ellipse_sim_DP.RST_p3_reps <- list()
...
for (s in 1:length(seeds)){
  ...
  results <- Ellipse_sim_BAST_p3_reps[[s]]
  ...
  Ellipse_sim_DP.RST_p3_reps[[s]] <- Ellipse_sim_DP.RST
  
  cat('Loop', s, 'done\n')
} #end of loop over seeds

## Save the results of the repetitive runs
save(Ellipse_sim_DP.RST_p3_reps, file = "/Ellipse_sim_DP.RST_p3_30reps.RData")
