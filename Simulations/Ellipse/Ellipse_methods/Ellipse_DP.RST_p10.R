...
### Load Data ###
Ellipse_sim_data <- readr::read_csv("/Ellipse_sim_data.csv")
head(Ellipse_sim_data)

load("/Ellipse_sim_BAST_p10_30reps.RData")
...
seeds <- c()
for (i in 1:length(Ellipse_sim_BAST_p10_reps)){
  seeds[i] <- Ellipse_sim_BAST_p10_reps[[i]][["seed"]]
}
seeds

print("Starting the loop")

Ellipse_sim_DP.RST_p10_reps <- list()
...
for (s in 1:length(seeds)){
  ...
  results <- Ellipse_sim_BAST_p10_reps[[s]]
  ...
  Ellipse_sim_DP.RST = DP.RST(Y_std, graph0, init_val, hyperpar,
                           MCMC, BURNIN, THIN,
                           PT = TRUE, PT_diff = 0.1, seed = seeds[s])
  
  Ellipse_sim_DP.RST$seed <- seeds[s]
  Ellipse_sim_DP.RST_p10_reps[[s]] <- Ellipse_sim_DP.RST
  
  cat('Loop', s, 'done\n')
} #end of loop over seeds

## Save the results of the repetitive runs
save(Ellipse_sim_DP.RST_p10_reps, file = "/Ellipse_sim_DP.RST_p10_30reps.RData")
