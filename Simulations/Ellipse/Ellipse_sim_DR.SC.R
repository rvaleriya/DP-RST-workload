...
Ellipse_sim_DR.SC_p3_reps <- list()

for (s in 1:length(seeds)){
  
  set.seed(seeds[s])
  
  DR.SC_fit <- DR.SC_fit(Y_std, K=5, Adj_sp=Adj_sp, q=p,
  ...
  Ellipse_sim_DR.SC_p3_reps[[s]] <- DR.SC_fit
}

# Save the results of the repetitive runs
save(Ellipse_sim_DR.SC_p3_reps, file = "./New_Simulations/Ellipse/Ellipse_results/Ellipse_sim_DR.SC_p3_30reps.RData")


#-------------------------------------------------------------------------------
# Extract first three PCAs
Y_sample = Ellipse_sim_data[, 5:14] # 5:7 for PCs 1-3 and 5:14 for PCs 1-10
head(Y_sample)
...
# Create a list to store results from different runs
Ellipse_sim_DR.SC_p10_reps <- list()
...
for (s in 1:length(seeds)){
  ...
  Ellipse_sim_DR.SC_p10_reps[[s]] <- DR.SC_fit
}

# Save the results of the repetitive runs
save(Ellipse_sim_DR.SC_p10_reps, file = "./New_Simulations/Ellipse/Ellipse_results/Ellipse_sim_DR.SC_p10_30reps.RData")
