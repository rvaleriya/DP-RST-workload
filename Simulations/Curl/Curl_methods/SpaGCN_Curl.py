...
SIM_LOG_FILENAME = "SpaGCN_analysis_curl_multi_seed_3p_10p.log"
...
def load_simulated_data(csv_path):
    ...
    required_cols = ['X', 'Y', 'PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8', 'PC9', 'PC10', 'super_cluster'] # Added 'cluster'
    ...
    adata = ad.AnnData(X=df[['PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8', 'PC9', 'PC10']].values)
    ...
    adata.obs['ground_truth_cluster'] = pd.Categorical(df['super_cluster'].values)
...
def run_spagcn_simulated(adata, adj, l, n_clusters, num_pcs, r_seed, t_seed, n_seed):
    ...
    """
    # Store the input features (PC1-PC10) in adata.obsm['X_pca']
    # This signals SpaGCN functions (like search_res and train) to use these pre-computed components.
    # Slice to use only the first num_pcs components
    print(f"Storing input features (first {num_pcs} PCs) into adata.obsm['X_pca']")
    adata.obsm['X_pca'] = adata.X[:, :num_pcs]
...
def main():
    ...
    # --- Configuration ---
    SIM_DATA_CSV = "/scratch/user/varogovchenko/BASTION_HPRC/Simulations/Curl/Curl_sim_data.csv"
    N_CLUSTERS = 10 # Number of clusters requested (used for kmeans init)
    NUM_PCS_LIST = [3, 10] # Run with both 3PCs and 10PCs
    OUTPUT_DIR = Path("./res_curl_data_3p_10p")
    ...
    # --- Loop through seeds and PC configurations ---
    # Store ARI scores separately for each PC configuration
    ari_scores_initial_3pcs = []
    ari_scores_refined_3pcs = []
    ari_scores_initial_10pcs = []
    ari_scores_refined_10pcs = []
    
    print(f"\n--- Starting Multi-Seed Analysis ({len(seeds_to_run)} seeds, {len(NUM_PCS_LIST)} PC configs) ---")
            ...
            try:
                ...
                if len(ground_truth_clusters) == len(initial_pred):
                    try:
                        ...
                        print(f"ARI Initial ({num_pcs} PCs, seed {seed}): {ari_initial:.4f}")
                        if num_pcs == 3:
                            ari_scores_initial_3pcs.append(ari_initial)
                        else:
                            ari_scores_initial_10pcs.append(ari_initial)
                    except ValueError as e:
                        print(f"Error calculating Initial ARI for {num_pcs} PCs, seed {seed}: {e}")
                else:
                     print(f"Error: Mismatch in length for Initial ARI ({num_pcs} PCs, seed {seed}).")

                if len(ground_truth_clusters) == len(refined_pred):
                    try:
                        ari_refined = adjusted_rand_score(ground_truth_clusters, refined_pred)
                        print(f"ARI Refined ({num_pcs} PCs, seed {seed}): {ari_refined:.4f}")
                        if num_pcs == 3:
                            ari_scores_refined_3pcs.append(ari_refined)
                        else:
                            ari_scores_refined_10pcs.append(ari_refined)
                    except ValueError as e:
                        print(f"Error calculating Refined ARI for {num_pcs} PCs, seed {seed}: {e}")
                else:
                     print(f"Error: Mismatch in length for Refined ARI ({num_pcs} PCs, seed {seed}).")

                # Add results to the main DataFrame with PC configuration in column name
                all_results_df[f'pred_{num_pcs}PCs_k{N_CLUSTERS}_seed_{seed}'] = adata_run.obs['pred']
                all_results_df[f'refined_pred_{num_pcs}PCs_k{N_CLUSTERS}_seed_{seed}'] = adata_run.obs['refined_pred']

            except Exception as e:
                print(f"Error during SpaGCN run for {num_pcs} PCs, seed {seed}: {e}. Skipping this configuration.")
                # Add NaN columns to indicate failure for this configuration
                all_results_df[f'pred_{num_pcs}PCs_k{N_CLUSTERS}_seed_{seed}'] = np.nan
                all_results_df[f'refined_pred_{num_pcs}PCs_k{N_CLUSTERS}_seed_{seed}'] = np.nan
                continue # Continue to the next PC configuration

    # --- Post-Loop Analysis ---
    print(f"\n{'='*20} Multi-Seed Analysis Complete {'='*20}")

    # 5. Calculate and Print ARI Statistics (Initial and Refined) for each PC configuration
    print("\nARI Statistics Across Seeds:")
    
    # 3PCs statistics
    print("\n  --- 3 PCs Configuration ---")
    if ari_scores_initial_3pcs:
        ari_initial_3pcs_np = np.array(ari_scores_initial_3pcs)
        print("  Initial Prediction ARI (3 PCs):")
        print(f"    Mean: {np.mean(ari_initial_3pcs_np):.4f}, StdDev: {np.std(ari_initial_3pcs_np):.4f}")
        print(f"    Min:  {np.min(ari_initial_3pcs_np):.4f}, Max:  {np.max(ari_initial_3pcs_np):.4f}")
        print(f"    Successful runs: {len(ari_initial_3pcs_np)}")
    else:
        print("  Initial Prediction ARI (3 PCs): No successful runs.")

    if ari_scores_refined_3pcs:
        ari_refined_3pcs_np = np.array(ari_scores_refined_3pcs)
        print("  Refined Prediction ARI (3 PCs):")
        print(f"    Mean: {np.mean(ari_refined_3pcs_np):.4f}, StdDev: {np.std(ari_refined_3pcs_np):.4f}")
        print(f"    Min:  {np.min(ari_refined_3pcs_np):.4f}, Max:  {np.max(ari_refined_3pcs_np):.4f}")
        print(f"    Successful runs: {len(ari_refined_3pcs_np)}")
    else:
        print("  Refined Prediction ARI (3 PCs): No successful runs.")

    # 10PCs statistics
    print("\n  --- 10 PCs Configuration ---")
    if ari_scores_initial_10pcs:
        ari_initial_10pcs_np = np.array(ari_scores_initial_10pcs)
        print("  Initial Prediction ARI (10 PCs):")
        print(f"    Mean: {np.mean(ari_initial_10pcs_np):.4f}, StdDev: {np.std(ari_initial_10pcs_np):.4f}")
        print(f"    Min:  {np.min(ari_initial_10pcs_np):.4f}, Max:  {np.max(ari_initial_10pcs_np):.4f}")
        print(f"    Successful runs: {len(ari_initial_10pcs_np)}")
    else:
        print("  Initial Prediction ARI (10 PCs): No successful runs.")

    if ari_scores_refined_10pcs:
        ari_refined_10pcs_np = np.array(ari_scores_refined_10pcs)
        print("  Refined Prediction ARI (10 PCs):")
        print(f"    Mean: {np.mean(ari_refined_10pcs_np):.4f}, StdDev: {np.std(ari_refined_10pcs_np):.4f}")
        print(f"    Min:  {np.min(ari_refined_10pcs_np):.4f}, Max:  {np.max(ari_refined_10pcs_np):.4f}")
        print(f"    Successful runs: {len(ari_refined_10pcs_np)}")
    else:
        print("  Refined Prediction ARI (10 PCs): No successful runs.")

    # 7. Generate and Save ARI Boxplots (Initial vs Refined for each PC configuration)
    # Plot for 3PCs
    if ari_scores_initial_3pcs or ari_scores_refined_3pcs:
        plot_data_3pcs = []
        plot_labels_3pcs = []
        if ari_scores_initial_3pcs:
            plot_data_3pcs.append(ari_scores_initial_3pcs)
            plot_labels_3pcs.append('Initial')
        if ari_scores_refined_3pcs:
            plot_data_3pcs.append(ari_scores_refined_3pcs)
            plot_labels_3pcs.append('Refined')

        if plot_data_3pcs:
            try:
                fig, ax = plt.subplots(figsize=(8, 6))
                bp = ax.boxplot(plot_data_3pcs, patch_artist=True, labels=plot_labels_3pcs)
                colors = ['lightblue', 'lightgreen']
                for patch, color in zip(bp['boxes'], colors[:len(bp['boxes'])]):
                    patch.set_facecolor(color)
                ax.set_ylabel("Adjusted Rand Index (ARI)")
                ax.set_title(f"ARI Distribution - 3 PCs (Across {len(seeds_to_run)} Seeds)")
                ax.grid(axis='y', linestyle='--', alpha=0.7)
                plt.tight_layout()
                boxplot_path = OUTPUT_DIR / "simulated_ari_boxplot_comparison_3pcs.png"
                plt.savefig(boxplot_path, dpi=300)
                print(f"Saved ARI comparison boxplot (3 PCs) to: {boxplot_path}")
                plt.close(fig)
            except Exception as e:
                print(f"Error generating or saving ARI comparison boxplot (3 PCs): {e}")

    # Plot for 10PCs
    if ari_scores_initial_10pcs or ari_scores_refined_10pcs:
        plot_data_10pcs = []
        plot_labels_10pcs = []
        if ari_scores_initial_10pcs:
            plot_data_10pcs.append(ari_scores_initial_10pcs)
            plot_labels_10pcs.append('Initial')
        if ari_scores_refined_10pcs:
            plot_data_10pcs.append(ari_scores_refined_10pcs)
            plot_labels_10pcs.append('Refined')

        if plot_data_10pcs:
            try:
                fig, ax = plt.subplots(figsize=(8, 6))
                bp = ax.boxplot(plot_data_10pcs, patch_artist=True, labels=plot_labels_10pcs)
                colors = ['lightblue', 'lightgreen']
                for patch, color in zip(bp['boxes'], colors[:len(bp['boxes'])]):
                    patch.set_facecolor(color)
                ax.set_ylabel("Adjusted Rand Index (ARI)")
                ax.set_title(f"ARI Distribution - 10 PCs (Across {len(seeds_to_run)} Seeds)")
                ax.grid(axis='y', linestyle='--', alpha=0.7)
                plt.tight_layout()
                boxplot_path = OUTPUT_DIR / "simulated_ari_boxplot_comparison_10pcs.png"
                plt.savefig(boxplot_path, dpi=300)
                print(f"Saved ARI comparison boxplot (10 PCs) to: {boxplot_path}")
                plt.close(fig)
            except Exception as e:
                print(f"Error generating or saving ARI comparison boxplot (10 PCs): {e}")

    # Combined comparison plot (3PCs vs 10PCs)
    if (ari_scores_refined_3pcs or ari_scores_refined_10pcs):
        plot_data_combined = []
        plot_labels_combined = []
        if ari_scores_refined_3pcs:
            plot_data_combined.append(ari_scores_refined_3pcs)
            plot_labels_combined.append('3 PCs')
        if ari_scores_refined_10pcs:
            plot_data_combined.append(ari_scores_refined_10pcs)
            plot_labels_combined.append('10 PCs')

        if plot_data_combined:
            try:
                fig, ax = plt.subplots(figsize=(8, 6))
                bp = ax.boxplot(plot_data_combined, patch_artist=True, labels=plot_labels_combined)
                colors = ['lightcoral', 'lightsteelblue']
                for patch, color in zip(bp['boxes'], colors[:len(bp['boxes'])]):
                    patch.set_facecolor(color)
                ax.set_ylabel("Adjusted Rand Index (ARI)")
                ax.set_title(f"ARI Distribution - Refined Predictions (3 PCs vs 10 PCs)")
                ax.grid(axis='y', linestyle='--', alpha=0.7)
                plt.tight_layout()
                boxplot_path = OUTPUT_DIR / "simulated_ari_boxplot_3pcs_vs_10pcs.png"
                plt.savefig(boxplot_path, dpi=300)
                print(f"Saved ARI comparison boxplot (3 PCs vs 10 PCs) to: {boxplot_path}")
                plt.close(fig)
            except Exception as e:
                print(f"Error generating or saving ARI comparison boxplot (3 PCs vs 10 PCs): {e}")

    # 8. Save combined results DataFrame to CSV
    results_csv_path = OUTPUT_DIR / "curl_spagcn_clustering_results_3p_10p_multi_seed.csv"
