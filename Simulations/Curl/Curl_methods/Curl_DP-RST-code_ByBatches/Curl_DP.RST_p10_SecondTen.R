...
load("/Curl_sim_BAST_p10_30reps.RData")
...
seeds <- c()
for (i in 1:length(Curl_sim_BAST_p10_reps)){
  seeds[i] <- Curl_sim_BAST_p10_reps[[i]][["seed"]]
}
seeds

print("Starting the loop")

Curl_sim_DP.RST_p10_reps <- list()
...
for (s in 11:20){
  ...
  results <- Curl_sim_BAST_p10_reps[[s]]
  ...
  Curl_sim_DP.RST_p10_reps[[s]] <- Curl_sim_DP.RST
  
  cat('Loop', s, 'done\n')
  
  cat('Save the current file\n')
  save(Curl_sim_DP.RST_p10_reps, file = "/Curl_sim_DP.RST_p10_SecondTen_30reps.RData")
