...
### Load Data ###
Curl_sim_data <- readr::read_csv("/Curl_sim_data.csv")
head(Curl_sim_data)

load("/Curl_sim_BAST_p3_30reps.RData")

# Extract first three PCAs
Y_sample = Curl_sim_data[, 5:7]
...
graph0 <- trim_graph(n = nrow(coords), mesh = mesh, threshold = 0.12)

# plot(graph, layout = as.matrix(loc), vertex.size = 1, vertex.label = NA, edge.width = 0.5)
...
seeds <- c()
for (i in 1:length(Curl_sim_BAST_reps)){
  seeds[i] <- Curl_sim_BAST_reps[[i]][["seed"]]
}
seeds

print("Starting the loop")

Curl_sim_DP.RST_p3_reps <- list()
...
for (s in 11:20){
  ...
  hyperpar = list()
  hyperpar[['sigmasq_mu']] = 0.1 # leave it to be as in univariate case (just in the code write as a matrix)
  ...
  Curl_sim_DP.RST = DP.RST(Y_std, graph0, init_val, hyperpar,
                           MCMC, BURNIN, THIN,
                           PT = TRUE, PT_diff = 0.1, seed = seeds[s])
  
  Curl_sim_DP.RST$seed <- seeds[s]
  Curl_sim_DP.RST_p3_reps[[s]] <- Curl_sim_DP.RST
  
  cat('Loop', s, 'done\n')
  cat('Save the current file\n')
  save(Curl_sim_DP.RST_p3_reps, file = "/Curl_sim_DP.RST_p3_SecondTen_reps.RData")
