...
clusters <- cutree(hc, k = 80)

# init_clust_plot <- ggplot() +
#   # geom_boundary(bnd) +
#   scale_y_reverse() +
#   geom_point(aes(x = X, y = Y, col = factor(clusters)), data = Curl_sim_data) +
...
BURNIN = 7000 # burnin period length
THIN = 5       # thinning intervals

dynamic_part <- "Curl_sim_rep"

seeds <- sample.int(1000, size = 30, replace = FALSE)
seeds

Curl_sim_BAST_reps <- list()

for (s in 1:length(seeds)){

  Curl_sim_BAST = fitBAST(Y_std, graph0, init_val, hyperpar, temp,
                      MCMC, BURNIN, THIN,
                      PT = FALSE, seed = seeds[s],
                      backup_d = sprintf("./Simulations/Curl/%s_backup.RData", dynamic_part))

  Curl_sim_BAST$seed <- seeds[s]

  Curl_sim_BAST_reps[[s]] <- Curl_sim_BAST

  cat('Loop', s, 'done\n')
}

# Save the results of the repetitive runs
save(Curl_sim_BAST_reps, file = "Simulations/Curl/Curl_sim_BAST_30reps.RData")
