...
clust_plot <- ggplot() +
  geom_boundary(bnd) +
  geom_point(aes(x = x, y = y, col = factor(spatial_cluster)), data = sim_data) +
  labs(x = 'Coordinate X', y = 'Coordinate Y', colour = "True label") +
...
# Initialize all points from one cluster only
cluster = matrix(rep(1), nrow = n, ncol = M) # initial cluster memberships
cluster_ids <- unique(cluster[,1])
k_max = 20 #length(cluster_ids) # maximum number of clusters
...
BURNIN = 3500 # burnin period length
THIN = 5       # thinning intervals

dynamic_part <- "SR_sim_rep"
...
for (s in 1:length(seeds)){
  ...
  SR_sim_PT = fitBAST(Y_std, graph0, init_val, hyperpar, temp, 
                      MCMC, BURNIN, THIN, 
                      PT = FALSE, seed = seeds[s],
                      backup_d = sprintf("./Simulations/Swiss_Roll_3k/%s_backup.RData", dynamic_part))
  SR_sim_PT$seed <- seeds[s]
  
  SR_sim_PT_reps[[s]] <- SR_sim_PT
}

# Save the results of the repetitive runs
save(SR_sim_PT_reps, file = "./Simulations/Swiss_Roll_3k/SR_sim_PT_30reps.RData")
