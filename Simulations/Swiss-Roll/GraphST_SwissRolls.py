...
log_file_path = "GraphST_SwissRoll_MultiSeed_output.log"
output_dir_csv = "res_swiss_roll_csv"
seeds_to_run = [
...
# --- Load Data ---
data_file = "/scratch/user/varogovchenko/BASTION_HPRC/Simulations/Swiss_Roll_3k/SR_sim_data.csv"
...
features = df[['Y1', 'Y2', 'Y3']].values
spatial_coords = df[['x', 'y']].values
observations = df[['cluster', 'spatial_cluster']].copy()
...
for seed in seeds_to_run:
    ...
    try:
        ...
        if 'cluster' in adata.obs.columns:
            ari = metrics.adjusted_rand_score(adata.obs['cluster'], adata.obs[cluster_key])
            all_aris[seed] = ari
...
final_df = pd.DataFrame({
    ...
    'original_cluster': adata_base.obs.get('cluster', pd.NA),
    'original_spatial_cluster': adata_base.obs.get('spatial_cluster', pd.NA),
})
...
output_path = os.path.join(output_dir_csv, f"graphst_swiss_roll_multiseed_k{num_clusters}_radius{graph_radius}_results.csv")
