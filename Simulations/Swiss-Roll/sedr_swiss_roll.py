...
log_file_path = "SEDR_SwissRoll_MultiSeed_output.log" # Updated log file name
...
num_clusters = 3 # Based on 'cluster' column in data
graph_neighbors = 30
...
observations = df[['cluster', 'spatial_cluster']].copy() # Keep original labels
...
final_df = pd.DataFrame({
    ...
    'original_spatial_cluster': adata.obs.get('spatial_cluster', pd.NA),
})
...
csv_filename = os.path.join(output_dir_csv, f"sedr_swiss_roll_multiseed_k{num_clusters}_results.csv")
