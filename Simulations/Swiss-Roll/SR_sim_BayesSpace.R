...
SR_sim_BayesSpace_reps <- list()

for (s in 1:length(seeds)){
  set.seed(seeds[s])
  
  BS_results <- spatialCluster(sce, use.dimred = "PCA", d = 3, q = 3, platform = "ST")
  
  SR_sim_BayesSpace_reps[[s]] <- BS_results
}

# Save the results of the repetitive runs
save(SR_sim_BayesSpace_reps, file = "./Simulations/Swiss_Roll_3k/SR_sim_BayesSpace_30reps.RData")
