...
U_sim_DR.SC_reps <- list()

for (s in 1:length(seeds)){
  
  set.seed(seeds[s])
  
  DR.SC_fit <- DR.SC_fit(Y_std, K=6, Adj_sp=Adj_sp, q=3,
  ...
  U_sim_DR.SC_reps[[s]] <- DR.SC_fit
}

# Save the results of the repetitive runs
save(U_sim_DR.SC_reps, file = "./Simulations/U-shape_6k/U_sim_DR.SC_30reps.RData")
