...
results_dir <- "DP.RST/Competing_Methods/STAGATE_runs/res_realdata_csv"
true_labels_base_dir <- "/DP.RST"

datasets <- list(
  Brain = list(
    results_file = file.path(results_dir, "stagate_brain_results.csv"),
    true_labels_file = file.path(true_labels_base_dir, "Brain", "brain_true_labels.csv"),
    true_label_col = "type"
  ),
  Breast = list(
    results_file = file.path(results_dir, "stagate_breast_results.csv"),
    true_labels_file = file.path(true_labels_base_dir, "Breast", "breast_true_labels.csv"),
    true_label_col = "z"
  ),
  Gut = list(
    results_file = file.path(results_dir, "stagate_gut_reduced_results.csv"),
    true_labels_file = file.path(true_labels_base_dir, "Gut", "gut_true_labels.csv"),
    true_label_col = "z"
  ),
  Prostate = list(
    results_file = file.path(results_dir, "stagate_prostate_results.csv"),
...
output_file <- "DP.RST/Competing_Methods/STAGATE_ARI_results.txt"
sink(output_file)
